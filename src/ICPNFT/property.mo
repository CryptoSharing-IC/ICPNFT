...
module Property {
    ...
    // Returns a subset of from properties based on  the give query
    ...
    public func get(properties: Properties, qs: [Query]) : Result.Result<Properties, Types.Error> {
        ...
        var ps = Buffer.Buffer<Property>(0);
    ...
    // Updates the given properties based on the give update query.
    public func update(properties: Properties, us: [Update]) : Result.Result<Properties, Types.Error> {
        ...
        for (u in us.vals()) {
            switch (m.get(u.name)) {
                ...
                case (? p) { 
                    // Can't update immutable property.
                    if (p.immutable) {
    ...
    private func fromMap(m: HashMap.HashMap<Text, Property>) : Properties {
        var buffer = Buffer.Buffer<Property>(m.size());
    ...
    // Specifies the list of properties that are queries
    ...
    // Specifes the properties that should be updated to a certain value
